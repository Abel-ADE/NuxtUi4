<template>
  <section class="flex flex-col gap-10">
    <component
      :is="heading"
      class="text-2xl md:text-3xl text-pretty tracking-tight font-bold text-highlighted text-center"
    >
      {{ title }}
    </component>
    ...
    <UPageGrid>
      <UPageCard
        v-for="scale in escalasMostradas"
        :key="scale.id"
        :title="scale.name"
        :description="scale.description"
        :to="'escalas/' + scale.slug"
        variant="subtle"
        :ui="{
          container:
            'shadow-lg rounded-lg dark:border-solid dark:border-gray-700 dark:border-2',
        }"
      >
        <template #header>
          <div class="flex gap-2">
            <UBadge
              ...
              :key="categories.id"
              :ui="{
                base: `text-${categories.category_color}-500 dark:text-${categories.category_color}-50 bg-${categories.category_color}-50 dark:bg-${categories.category_color}-900`,
              }"
              size="md"
              variant="soft"
            >
...
const { data: escalas } = await useAsyncData<Scale[]>(
  `allScales-limit-${props.limit}`,
  () =>
    $fetch(
      `https://nwtzbqotvsejuicatrzm.supabase.co/rest/v1/scales?select=*,categories_scales(id, id_scale, categories(*))&${limitUrl.value}`,
      {
        headers: {
          apikey:
...
const { data: categorias } = await useAsyncData<Category[]>("categories", () =>
  $fetch(
    "https://nwtzbqotvsejuicatrzm.supabase.co/rest/v1/categories?select=*",
    {
      headers: {
        apikey:
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im53dHpicW90dnNlanVpY2F0cnptIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTcwODE4MTMsImV4cCI6MjA3MjY1NzgxM30.Dhf8n6f3rlrTDNu3CPQt-gZbq9zlDIofH58pykaHLpY",
